...
<div class="three-column-layout">
    ...
    <div class="center-segment">
        ...
        <div class="progress-bars">
            ...
            <div class="discount-container" id="discount-container">
                ...
                <div class="discount-message" id="discount-message">
                    Наберите 7000 ₽ для активации скидки 5%
                </div>
            </div>
        </div>
        ...
        <div class="recent-orders-section">
            ...
            <a href="/personal/order/" class="view-all-link">Все заказы →</a>
        </div>
    </div>
    
    <!-- ПРАВЫЙ СЕГМЕНТ: Свиток -->
    <div class="right-segment">
        <!-- Верхний рулон пергамента -->
        <div class="scroll-top-roller">
            <img src="/upload/scroll-top.png" alt="пергамент" class="scroll-roller-img">
        </div>
        ...
        <!-- Нижний рулон пергамента -->
        <div class="scroll-bottom-roller">
            <img src="/upload/scroll-bottom.png" alt="пергамент" class="scroll-roller-img">
        </div>
    </div>
</div>
...
<style>
    ...
    * {
    ...
    .scroll-top-roller:hover .scroll-roller-img,
    .scroll-bottom-roller:hover .scroll-roller-img {
        transform: scale(1.02);
    }

    /* Анимация для рулонов при наведении на свиток */
    .right-segment:hover .scroll-top-roller .scroll-roller-img {
        transform: translateY(-3px);
    }

    .right-segment:hover .scroll-bottom-roller .scroll-roller-img {
        transform: translateY(3px);
    }
    ...
    .discount-progress {
        height: 100%;
        background: linear-gradient(90deg, var(--DragonLight), #f39c12);
        width: 0%;
    ...
    .recent-order-card {
        ...
        padding: 12px;
        transition: all 0.3s;
        border-left: 3px solid var(--DragonLight);
    }
    ...
    .status-cancelled { background: var(--DragonLight); color: white; }
    ...
    .scroll-seal {
        ...
        width: 45px;
        height: 45px;
        background: radial-gradient(circle, var(--DragonLight) 30%, #c0392b 70%);
        border-radius: 50%;
        display: flex;
        align-items: center;
        justify-content: center;
    ...
    /* Стили для комментария - напрямую по id */
    #comment {
        width: 100%;
        padding: 10px 12px;
        border: 1px solid #e0cba0;
    ...
    .btn-submit-order {
        width: 100%;
        padding: 10px;
        background: linear-gradient(135deg, var(--DragonLight), #c0392b);
        color: white;
        border: none;
        border-radius: 8px;
        font-size: 13px;
    ...
    .error {
        border-color: var(--DragonLight) !important;
    }
</style>
...
<script>
    ...
    // Переменная для хранения последнего удаленного товара
    // Переменная для хранения последнего удаленного товара
    let lastRemovedItem = null;
    let undoTimeout = null;

    // Показать уведомление с кнопкой "Отменить" (5 секунд)
    function showUndoNotification(item) {
        ...
        const oldUndo = document.querySelector('.undo-notification');
        if (oldUndo) oldUndo.remove();
        
        // Создаем уведомление с таймером
        let timeLeft = 5;
        ...
        undoDiv.innerHTML = `
            <div style="
                ...
                font-size: 14px;
            ">
                <span>💨 ${item.name} выпал из корзины</span>
                <span id="undoTimer" style="background: rgba(255,255,255,0.2); padding: 2px 8px; border-radius: 20px;">5</span>
                <button onclick="undoRemoveItem()" style="
                    background: #f39c12;
                    border: none;
                    color: white;
                    padding: 5px 15px;
        ...
        const timerSpan = undoDiv.querySelector('#undoTimer');
        const timerInterval = setInterval(() => {
            ...
            if (timeLeft <= 0) {
                ...
                const undo = document.querySelector('.undo-notification');
                if (undo) undo.remove();
    ...
    // Функция отмены удаления - возврат НАД КОРЗИНОЙ
    window.undoRemoveItem = function() {
        if (!lastRemovedItem) return;
        
        console.log('↩️ Возвращаем товар над корзиной:', lastRemovedItem.name);
        ...
        // Получаем позицию корзины для спавна НАД ней
        const basketRect = document.getElementById('basket').getBoundingClientRect();
        const spawnX = basketRect.left + basketRect.width / 2 + (Math.random() - 0.5) * 60;
        const spawnY = basketRect.top - 50; // Над корзиной
        
        // Проверяем, есть ли уже такой товар в cartProducts
        const existingProduct = cartProducts.find(p => p.id == lastRemovedItem.productId);
        if (!existingProduct) {
            // Добавляем в cartProducts, если нет
            cartProducts.push({
                id: lastRemovedItem.productId,
                name: lastRemovedItem.name,
                price: lastRemovedItem.price,
                imageUrl: lastRemovedItem.imageUrl
            });
            console.log('✅ Товар добавлен в cartProducts:', lastRemovedItem.name);
        }
        
        // Обновляем отображение скроллера ДО создания товара
        renderScrollItems();
        
        // Создаем товар НАД корзиной
        const restoredItem = createProduct({
            id: lastRemovedItem.productId,
            name: lastRemovedItem.name,
            price: lastRemovedItem.price,
            imageUrl: lastRemovedItem.imageUrl
        ...
        // Добавляем небольшое падение
        if (restoredItem) {
            restoredItem.vy = 2;
            restoredItem.vx = (Math.random() - 0.5) * 1.5;
        }
        
        // Обновляем корзину Битрикса
        fetch('/ajax/add_to_cart.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            body: `action=add&product_id=${lastRemovedItem.productId}&quantity=1`
        });
        
        // Еще раз обновляем скроллер после создания товара (чтобы обновить количество)
        setTimeout(() => {
            renderScrollItems();
            updateBasketStats();
        }, 100);
        ...
        const restoredNotif = document.createElement('div');
        restoredNotif.innerHTML = `<div style="
            position: fixed;
            bottom: 100px;
            left: 50%;
            transform: translateX(-50%);
            ...
            font-size: 12px;
            animation: slideUp 0.3s ease;
        ">⬆️ ${lastRemovedItem.name} возвращен в корзину</div>`;
        document.body.appendChild(restoredNotif);
        setTimeout(() => restoredNotif.remove(), 2000);
        
        lastRemovedItem = null;
    };

    // Также добавим функцию для возврата товара при перетаскивании вверх
    function returnItemToTop(item) {
        if (!item || item.pendingRemoval) return;
        
        console.log('⬆️ Возвращаем товар наверх:', item.name);
        ...
        function animateReturn(currentTime) {
            ...
            const newY = startY - (startY - endY) * easeOutQuad;
            
            item.y = newY;
            item.vy = 0;
            item.vx = item.vx * 0.95;
            ...
            if (progress < 1) {
                ...
                // Небольшой эффект при приземлении наверх
                item.element.style.transform = 'scale(1.05)';
                setTimeout(() => {
                    if (item.element) item.element.style.transform = '';
                }, 200);
            }
        }
    ...
    // Добавляем возможность вернуть товар наверх при перетаскивании вверх экрана
    window.addEventListener('mouseup', (e) => {
        if (isDragging && draggingItem) {
            // Если товар перетащили в верхнюю часть экрана (выше 100px)
            const rect = draggingItem.element.getBoundingClientRect();
            if (rect.top < 100 && !isItemInBasket(draggingItem)) {
                console.log('⬆️ Товар перетащен наверх, возвращаем...');
                returnItemToTop(draggingItem);
            }
    ...
    function applyPhysics() {
        for (let i = 0; i < items.length; i++) {
            ...
            // Получаем границы корзины
            const { bottomBounds, leftPoints, rightPoints } = getRodBounds();
            
            // Проверяем, находится ли товар в корзине (между стенками)
            let isInsideBasket = false;
            if (bottomBounds && leftPoints && rightPoints) {
                const t = Math.max(0, Math.min(1, (item.y - leftPoints.top.y) / (leftPoints.bottom.y - leftPoints.top.y)));
                const leftBoundary = leftPoints.top.x + (leftPoints.bottom.x - leftPoints.top.x) * t;
                const rightBoundary = rightPoints.top.x + (rightPoints.bottom.x - rightPoints.top.x) * t;
                const itemLeft = item.x - halfSize;
                const itemRight = item.x + halfSize;
                
                // Товар в корзине, если между стенками И выше дна
                isInsideBasket = (itemRight > leftBoundary - 20 && itemLeft < rightBoundary + 20) &&
                                (item.y + halfSize <= bottomBounds.top + 80);
            }
            
            // ЗОНА СМЕРТИ: нижняя часть экрана (от 80% высоты)
            const deathZoneStart = window.innerHeight - 150;
                    
            // Если товар НЕ в корзине И попал в зону смерти - удаляем
            if (!isInsideBasket && itemBottom >= deathZoneStart) {
                console.log('💨 Товар выпал из корзины и попал в зону смерти:', item.name);
            ...
            if (item.y - halfSize < 0) {
                item.y = halfSize;
                if (item.vy < 0) item.vy = -item.vy * 0.3;
            }
            ...
            // Проверка коллизии с трапецией (для товаров в корзине)
    ...
    // Проверка, находится ли товар внутри корзины
    function isItemInBasket(item) {
        const { bottomBounds, leftPoints, rightPoints } = getRodBounds();
        if (!bottomBounds || !leftPoints || !rightPoints) return false;
        ...
        // Проверяем по высоте: товар должен быть на уровне дна корзины
        if (itemBottom < bottomBounds.top - 20 || itemBottom > bottomBounds.bottom + 20) {
            return false;
        }
        
        // Проверяем по ширине: товар должен быть между левой и правой стенкой
        const t = Math.max(0, Math.min(1, (itemCenterY - leftPoints.top.y) / (leftPoints.bottom.y - leftPoints.top.y)));
        const leftBoundary = leftPoints.top.x + (leftPoints.bottom.x - leftPoints.top.x) * t;
        const rightBoundary = rightPoints.top.x + (rightPoints.bottom.x - rightPoints.top.x) * t;
        ...
        return itemLeft >= leftBoundary - 20 && itemRight <= rightBoundary + 20;
    }
    ...
    function removeItem(item) {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // НЕ УДАЛЯЕМ из cartProducts, даже если осталось 0!
                // Мы оставляем товар в скроллере, чтобы можно было его вернуть через "Отменить"
                // Количество показываем 0, но товар остается в списке
    ...
    const DISCOUNT_THRESHOLD = 7000; // Порог для скидки 5000₽
    const DISCOUNT_PERCENT = 5; // Скидка 5%

    function updateDiscount(total) {
        // Проверяем, достигнута ли сумма порога
        const isDiscountActive = total >= DISCOUNT_THRESHOLD;
        ...
        if (discountPercentSpan) {
            discountPercentSpan.innerText = discountPercent > 0 ? discountPercent + '%' : '0%';
        }
        
        // Обновляем отображение суммы скидки
        if (discountAmountSpan) {
            discountAmountSpan.innerText = discountAmount.toLocaleString() + ' ₽';
        }
        
        // Прогресс бар - показываем прогресс до порога
        const progress = Math.min((total / DISCOUNT_THRESHOLD) * 100, 100);
        if (discountProgressBar) {
            discountProgressBar.style.width = progress + '%';
            
            // Меняем цвет в зависимости от прогресса
            if (progress >= 100) {
                discountProgressBar.style.background = 'linear-gradient(90deg, #27ae60, #2ecc71)';
            } else {
                discountProgressBar.style.background = 'linear-gradient(90deg, var(--DragonLight), #f39c12)';
            }
        }
        ...
        if (discountMessage) {
            if (progress >= 100) {
                discountMessage.innerHTML = '🎉 Скидка 5% активирована! 🎉';
                discountMessage.style.color = '#27ae60';
            } else {
                const need = DISCOUNT_THRESHOLD - total;
                discountMessage.innerHTML = `Добавьте еще ${need.toLocaleString()} ₽ для активации скидки 5%`;
                discountMessage.style.color = 'var(--DragonLight)';
            }
        }
    ...
    // ========== СОЗДАНИЕ ТОВАРА ==========
    // ========== СОЗДАНИЕ ТОВАРА ==========
    function createProduct(productData, customX = null, customY = null) {
        ...
        // Если передан customY - используем его (для падения с верха)
        if (customY !== null) {
            if (customX !== null) {
                spawnX = customX;
            } else if (basketRect && basketRect.top > 0) {
        ...
        // Если передан только customX
        else if (customX !== null) {
            spawnX = customX;
            spawnY = -30;
        }
        // Обычное создание - над корзиной
        else if (basketRect && basketRect.top > 0) {
            const offsetX = (Math.random() - 0.5) * 80;
            const offsetY = -30;
            spawnX = basketRect.left + basketRect.width / 2 + offsetX;
    ...
    // Запуск падения всех товаров
    function startFallAnimation() {
        fetch('/ajax/add_to_cart.php?action=get_full&t=' + Date.now())
            .then(response => response.json())
            .then(data => {
                if (data.success && data.items) {
                    // Собираем все товары в очередь
                    data.items.forEach(item => {
                        if (!item.NAME || item.NAME.trim() === '' || item.PRICE === 0) return;
                    ...
                    console.log(`📦 В очереди ${fallQueue.length} товаров`);
                    ...
                    // Запускаем падение первого
                    processNext();
                }
            });
    }

    // Падение следующего товара
    function processNext() {
        if (fallQueue.length === 0) {
            console.log('✅ Все товары упали');
            isFallingNow = false;
            return;
        }
        
        if (isFallingNow) return;
        isFallingNow = true;
        
        const product = fallQueue.shift();
        console.log(`⬇️ Падает: ${product.name} (осталось: ${fallQueue.length})`);
        
        // Добавляем в cartProducts
        const exists = cartProducts.some(p => p.id == product.id);
        if (!exists) {
            cartProducts.push({
                id: product.id,
                name: product.name,
        ...
        // СОЗДАЕМ ТОВАР С ВЕРХА ЭКРАНА (Y = 40)
        createProduct({
            id: product.id,
            name: product.name,
            price: product.price,
            imageUrl: product.imageUrl
        }, null, 40); // ← вот здесь задаем высоту появления!
        
        // Ждем 500мс и запускаем следующий
        setTimeout(() => {
            isFallingNow = false;
            processNext();
        }, 500);
    ...
    function updateCartTotal(total) {
        ...
        // Скидка применяется только если прогресс 100%
        const isDiscountActive = total >= DISCOUNT_THRESHOLD;
        const finalTotal = isDiscountActive ? total - discountAmount : total;
    ...
    class StabilitySystem {
        ...
        startStabilityCheck() {
            ...
            this.checkInterval = setInterval(() => {
                // Если корзина пуста - не обновляем стабильность
                if (items.length === 0) {
        ...
        observeCartChanges() {
            window.addEventListener('cartItemAdded', () => {
                // Если товар добавили - сбрасываем стабильность
                this.resetStability();
            });
            window.addEventListener('cartItemRemoved', () => {
                // Если товар удалили - сбрасываем стабильность
                this.resetStability();
            });
            window.addEventListener('cartItemMoved', () => {
                // Если товар двигают - сбрасываем стабильность
                this.resetStability();
            });
        }
        
        resetStability() {
            // Если корзина пуста - не запускаем таймер
            if (items.length === 0) {
                this.isStable = false;
                this.stabilityStartTime = null;
                this.updateProgressBar(0);
        ...
        updateProgressBar(progress) {
            ...
            if (timerElement && progress < 100 && progress > 0) {
            ...
            } else if (timerElement && progress === 0 && items.length === 0) {
                timerElement.textContent = '0 сек';
            } else if (timerElement && progress >= 100) {
        ...
        disableCheckoutButton() {
            ...
            if (hint) hint.innerHTML = items.length === 0 ? '🛒 Добавьте товары в корзину' : '🔒 Удерживайте товары 5 секунд для активации кнопки';
        }
    ...
    function submitOrder(event) {
        ...
        const score = document.getElementById('player-score')?.textContent || '0';
    ...
    function showNotification(message, type) {
        ...
        notif.style.background = type === 'success' ? '#27ae60' : type === 'warning' ? '#f39c12' : 'var(--DragonLight)';
    ...
    function updateScreenBottom() {
        ...
        console.log('screenBottom обновлен:', screenBottom);
    }
    ...
    function init() {
        ...
        // НЕ загружаем товары из localStorage и Битрикса обычным способом
        // loadPendingCartItems(); // закомментировать
        // loadCartFromBitrix();   // закомментировать
